...
def create_lvl_matrix(lvl_x, lvl_y, player_vis_lvl):
	...
	for line in lvl:
		...
		for element in line:
			...
			if randnum > 80:
				lvl[y][x] = random.choice([(0, 0), (random.randint(0, 2), random.randint(0, 2) )])
			if randnum < 5:
...
class game():
	def __init__(self):
		...
		self.all_enemys = [test_enemy, enemy_1, enemy_2]
...
class enemy_1(test_enemy):
	enemy_id = 1
	def __init__(self, x, y, enemy_manager):
		test_enemy.__init__(self, x, y, enemy_manager)
		self.image = pygame.Surface((16,16))
		self.image.fill((255,0,0))


class enemy_2(test_enemy):
	enemy_id = 2
	def __init__(self, x, y, enemy_manager):
		test_enemy.__init__(self, x, y, enemy_manager)
		self.image = pygame.Surface((16,16))
